...
def run_rfi_assistant():
    ...
    with col2:
        ...
        if use_smart != st.session_state.use_smart_api:
    ...
    for idx, message in enumerate(st.session_state.rfi_chat):
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "assistant" and message.get("api_used"):
                api_indicator = "🧠 Smart Search" if message["api_used"] == "smart" else "⚡ Quick Search"
                st.caption(f"*Generated by {api_indicator}*")
                
                # Add retry button for Quick Search responses in chat history
                if message["api_used"] == "rfi_agent" and message.get("original_prompt"):
                    if st.button("🧠 Retry with Smart Search", key=f"retry_history_{idx}"):
    ...
    if "retry_prompt" in st.session_state:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Retrying with Smart Search..."):
                ...
                try:
                    ...
                    if isinstance(smart_result, list) and len(smart_result) > 0:
                        ...
                        if smart_answer:
                            st.markdown("**🧠 Smart Search Response:**")
                            ...
                            st.error("❌ Smart Search returned no answer")
                    else:
                        st.error("❌ Invalid Smart Search response format")
                
                except Exception as smart_err:
                    st.error(f"❌ Smart Search Error: {smart_err}")
            ...
            with st.spinner("Analyzing documents..."):
                ...
                try:
                    ...
                    if answer:
                        ...
                        # Add "Retry with Smart Search" button for Quick Search responses
                        if not st.session_state.use_smart_api:
                            if st.button("🧠 Retry with Smart Search", key=f"retry_{len(st.session_state.rfi_chat)}"):
                                with st.spinner("Analyzing with Smart Search..."):
